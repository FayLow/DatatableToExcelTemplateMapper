...
namespace Util
{
    public static class OpenXMLUtil
    {
        ...
        public static string ReadCell(WorkbookPart wbPart, Cell iCell)
        {
            string text = string.Empty;

            if (iCell.DataType != null)
            {
                if (iCell.DataType == CellValues.SharedString)
                {
                    int id = -1;

                    if (Int32.TryParse(iCell.InnerText, out id))
                    {
        ...
        public static void RemoveRow(Worksheet worksheet, uint rowIndex)
        {
            Row row = GetRow(worksheet, rowIndex);
            row.Remove();

            UpdateRowIndexes(worksheet.WorksheetPart, rowIndex, true);
            UpdateMergedCellReferences(worksheet.WorksheetPart, rowIndex, true);
        }
        ...
        public static void UpdateRowIndexes(WorksheetPart worksheetPart, uint rowIndex, bool isDeletedRow)
        {
        ...
        public static void UpdateMergedCellReferences(WorksheetPart worksheetPart, uint rowIndex, bool isDeletedRow)
        {
            if (worksheetPart.Worksheet.Elements<MergeCells>().Count() > 0)
            {
                ...
                if (mergeCells != null)
                {
                    // Grab the index row merged cells first. Will add back after.
                    List<MergeCell> mergeCellsListToAdd = new List<MergeCell>();
                    ...
                    {
                        if (GetRowIndex(mergeCell.Reference.Value.Split(':').ElementAt(0)) == rowIndex ||
                            GetRowIndex(mergeCell.Reference.Value.Split(':').ElementAt(1)) == rowIndex)
                        {
                            mergeCellsListToAdd.Add((MergeCell)mergeCell.Clone());
                        }
                    ...
                    if (!isDeletedRow)
                    {
                        foreach (MergeCell mergeCellToAdd in mergeCellsListToAdd)
                        {
                            mergeCells.Append(mergeCellToAdd);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// ref: https://stackoverflow.com/questions/8207869/how-to-export-datatable-to-excel
